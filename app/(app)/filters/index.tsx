...
const Filters: FC<FiltersProps> = () => {
  ...
  const itemClick = (name, key) => {
    if (!current?.[name] || current?.[name]?.findIndex(filterItem => filterItem === key) === -1) {
      addFilter(name, key)
    } else {
  ...
  const addFilter = (name, key) => {
    ...
    setCurrent({ ...(current || {}), [name]: [...(current?.[name] || []), key] })
  }
  const removeFilter = (name, key) => {
    ...
    setCurrent({ ...(current || {}), [name]: [...(current?.[name]?.filter(v => v !== key) || [])] })
  }
                    ...
                    >
                      <View style={styles.filterTypeItemLeft}>
                        <Checkbox
                          value={
                            current?.[v?.name] &&
                            current?.[v?.name]?.findIndex(filterItem => filterItem === item?.id) !== -1
                          }
                          onChange={checked => {
                            if (checked) {
