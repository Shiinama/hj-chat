...
export class SocketStream {
  ...
  private onTextStreamUpdate: {
    [key: string]: (item: MessageStreamText) => void;
  } = {};

  private onAudioStreamUpdate: {
    [key: string]: (
      item: MessageStreamText,
      url: string,
      timeout?: boolean,
    ) => void;
  } = {};
  ...
  init() {
    ...
    this.socket.on(MsgEvents.MSG_TEXT_STREAM, (e) =>
      this.onMessageTextStream(e),
    );
    this.socket.on(MsgEvents.MSG_AUDIO_STREAM, (e) =>
      this.onMessageAudioStream(e),
    );
    ...
    this.socket.on(MsgEvents.MSG_TRANSLATED, (e) =>
      this.onMessageTranslated(e),
    );
    this.socket.on(MsgEvents.REPLY_MESSAGE_CREATED, (e) =>
      this.onMessageReplyCreated(e),
    );
    this.socket.on(MsgEvents.RESET_MEMORY, (e) =>
      this.onMessageContextClear(e),
    );
  ...
  private onMessageTextStream(data: MessageStreamTextRes) {
    if (this.currentBot?.botBaseInfo.id !== data.data?.replyMessage?.botId)
      return;
  ...
  async PlatformAudioSet(data, msgKey) {
    if (Platform.OS === "ios") {
      ...
      return `data:audio/mp3;base64,${arrayBufferToBase64(
        this.currentAudioStream[msgKey],
      )}`;
    }
  }
  ...
  private async onMessageAudioStream(data: MessageStreamTextRes) {
    ...
    try {
      ...
      // 待回复消息的机器人和哪一条消息
      const msgKey =
        msg.replyMessage?.botId + "&BOT&" + msg.replyMessage?.replyUid;
      if (!msg.isFinal && msg.index === 0) {
        ...
        this.currentAudioStream[msgKey] = concatBuffer(
          this.currentAudioStream[msgKey],
          msg.audio,
        );
      }
  ...
  playStreamNext() {
    if (
      this.audioStreamPlayKeys?.length > 0 &&
      this.audioStreamIndex < this.audioStreamPlayKeys.length - 1
    ) {
      this.audioStreamIndex += 1;
      CallBackManagerSingle().execute(
        "play_" + this.audioStreamPlayKeys[this.audioStreamIndex],
      );
    } else {
      this.audioStreamIndex = -1;
  ...
  private addTextStream(msgRes: MessageStreamTextRes) {
    ...
    const resMsg = {
      ...msg,
      text: this.currentTextStream[msgKey],
      msgLocalKey: msgKey,
    };
  ...
  addTextStreamCallBack(
    key: string,
    callBack: (item: MessageStreamText) => void,
  ) {
    delete this.onTextStreamUpdate[key];
    this.onTextStreamUpdate[key] = callBack;
  }

  addAudioStreamCallBack(
    key: string,
    callBack: (item: MessageStreamText, url: string, timeout?: boolean) => void,
  ) {
